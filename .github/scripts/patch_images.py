...
start_marker = '    // На мобильных в категориях full-фото грузим только рядом с экраном и по\n'
end_marker = '    // Дополнительный раздел «Список ожидания» внутри избранного. Загружаем\n'
start = s.index(start_marker)
end = s.index(end_marker, start)
replacement = r'''    // Карточки товаров теперь используют одну и ту же резкую CDN-версию:
...
s = s.replace('/favorites-waitlist.js?v=20260910a', '/favorites-waitlist.js?v=20260911cdn1')
card.write_text(s, encoding='utf-8')

waitlist = Path('favorites-waitlist.js')
w = waitlist.read_text(encoding='utf-8')
old_start = '    // На iPhone/Android карточки категории сразу показывают лёгкую miniature.\n'
old_end = '    function getUserId() {\n'
if old_start in w:
    a = w.index(old_start)
    b = w.index(old_end, a)
    w = w[:a] + w[b:]
