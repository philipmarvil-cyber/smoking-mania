...
start_marker = '    // Карточки товаров теперь используют одну и ту же резкую CDN-версию:\n'
end_marker = '    // Дополнительный раздел «Список ожидания» внутри избранного. Загружаем\n'
start = s.index(start_marker)
end = s.index(end_marker, start)
replacement = r'''    // Instant-first: miniature никогда не убираем, пока резкая CDN-картинка
...
b = index.read_bytes()
replacements = [
    (b"const LOCAL_CATALOG_KEY = 'catalog_cache_v2';", b"const LOCAL_CATALOG_KEY = 'catalog_cache_v3';"),
    (b"img: prod.img || '',", b"img: prod.img || ((Number(prod.imageCount) > 0 && prod.imageVersion && String(prod.imageVersion) !== '0') ? `/api/product-image?id=${encodeURIComponent(prod.id)}&v=${encodeURIComponent(prod.imageVersion)}` : ''),"),
    (b"/card-quality.js?v=20260911cdn1", b"/card-quality.js?v=20260911instant2")
]
for old, new in replacements:
    if old not in b:
        raise SystemExit(f'index marker missing: {old!r}')
