...
// Раньше стояло с большим запасом (2 параллельных, ~8 запросов/сек) —
// с каталогом в 7000+ товаров полная синхронизация не укладывалась в
// 60-секундный лимит функции на Vercel и обрывалась по таймауту, из-за
// чего кэш переставал обновляться вообще. Подняли ближе к реальному
// лимиту склада, чтобы синхронизация укладывалась по времени.
const MS_MAX_CONCURRENT = 5;
const MS_MIN_INTERVAL_MS = 70; // ~14 запросов/сек, чуть с запасом от 15/сек лимита
...
// раньше грузили строго по одной ("может позволить себе быть небыстрой"),
// и с каталогом в 7000+ товаров это упиралось в 60-секундный лимит
// функции на Vercel: синхронизация обрывалась по таймауту и кэш переставал
// обновляться вообще. Параллельная загрузка страниц ускоряет это в разы.
